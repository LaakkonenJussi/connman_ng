...
static void remove_resolv(struct wireguard_info *info)
{
	...
	if (info->resolv && info->resolv_id) {
		...
		g_resolv_cancel_lookup(info->resolv, info->resolv_id);
	}

	info->resolv_id = 0;
	info->remove_resolv_id = 0;

	g_resolv_unref(info->resolv);
	info->resolv = NULL;
}
